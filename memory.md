...
* **State**: Migrations `001` through `008` defined and verified.
* **Migrator**: Programmatic TypeScript runner (`database/migrator.ts`) with offline validation capabilities.

### 4.5 Subscriptions & Channel Entitlements (`/stockiq/modules/subscriptions/`)
* **State**: Completed Phase 10 implementation.
* **Tables**: `subscriptions` and `subscription_events` in migration `008_subscriptions.sql`.
* **Entitlement Gate**: `CAN_ACCESS_SERVICE_CHANNEL` strictly enforces `ACTIVE` status, validity timeframe, executed client agreement, and verified investor suitability before granting access.
* **REST API**: Mounted at `/api/v1/subscriptions` with complete RBAC and audit logging.
