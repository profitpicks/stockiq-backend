...
* [x] **Phase 10: Subscriptions & Channel Entitlements Engine** (Completed)
  * Implemented database migration `008_subscriptions.sql` with normalized `subscriptions` and immutable `subscription_events` tables.
  * Implemented 8 distinct lifecycle statuses: `PENDING`, `ACTIVE`, `EXPIRING`, `EXPIRED`, `CANCELLED`, `SUSPENDED`, `REFUND_PENDING`, `REFUNDED`.
  * Integrated server-side authoritativeness requiring paid payment order, executed agreement signature, and valid suitability assessment.
  * Implemented `CAN_ACCESS_SERVICE_CHANNEL` entitlement verification engine enforcing non-automatic access even if payment exists.
  * Implemented Express REST routes at `/api/v1/subscriptions` with RBAC authorization guards and idempotency protection.
  * Verified with passing unit test suite `phase10_subscriptions.test.ts`.

---

## 4. Next Scheduled Milestone (NEXT)

> **CRITICAL RULE**: The next functional phase must be selected deliberately by the user from the remaining roadmap.

### Candidate Next Milestones:
1. **Option A: Realtime Channel Broadcast & Notifications (Phase 11)**
   * High-speed WebSockets / SSE channel dispatcher distributing approved recommendations to subscribed investors.
2. **Option B: Grievance Redressal & SLA Monitoring (Phase 12)**
   * Investor complaint submission, ticket categorization, resolution timelines, and automated SLA breach alerts.
3. **Option C: Android Screen Expansion (Directory, Subscriptions & Channel Flows)**
   * Building client-side Compose screens for Public Provider Directory, Subscription checkout/management, and Service Channels.
